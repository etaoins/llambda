...
case class MutateVar(variable : BoundValue, value : Expression) extends Expression
...
case class Let(bindings : List[(BoundValue, Expression)], innerExprs : List[Expression]) extends Expression
