(define-test "datum can be cast to vector" (expect 1
  ; This assumes (vector-ref) takes a vector
...
(define-test "#true can be unboxed as truthy" (expect #f
  ; This assumes (not) takes a truthy
  (not (typeless-cell #t))))
...
(define-test "integer can be unboxed as native integer" (expect #(#t #t #t)
  ; This assumes (make-vector) takes a native integer length
...
(define-test "integer cannot be passed to a procedure as float" (expect-error type-error?
  (import (llambda nfi))
...
(define-test "types that cannot be boolean evaluate as true" (expect #f
  ; This assumes (not) takes an native boolean and (cons) returns a pair
  (not (cons 1 2))))
...
(define-test "converting an integer too large for <native-uint32> fails" (expect-error type-error?
  ; This assumes (make-list) takes a <native-uint32> length
...
(define-test "converting an integer too large for <native-uint8> fails" (expect-error type-error?
  ; This assumes (make-bytevector) takes a <native-uint8> fill
