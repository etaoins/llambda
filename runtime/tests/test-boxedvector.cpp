...
void testFromFill()
{
	...
	{
		BoxedVector *unspecificVector  = BoxedVector::fromFill(5);

		ASSERT_EQUAL(unspecificVector->length(), 5);
		ASSERT_EQUAL(unspecificVector->elementAt(0), BoxedUnspecific::instance());
		ASSERT_EQUAL(unspecificVector->elementAt(4), BoxedUnspecific::instance());
		ASSERT_EQUAL(unspecificVector->elementAt(5), 0);
	}
	
	{
		BoxedString *testString = BoxedString::fromUtf8CString(u8"Hello!");
		BoxedVector *stringVector  = BoxedVector::fromFill(4, testString);
...
{
	BoxedString *string1 = BoxedString::fromUtf8CString(u8"One");
	BoxedVector *vector1 = BoxedVector::fromFill(3, string1);

	BoxedString *string2 = BoxedString::fromUtf8CString(u8"Two");
	BoxedVector *vector2 = BoxedVector::fromFill(1, string2);

	BoxedString *string3 = BoxedString::fromUtf8CString(u8"Three");
	BoxedVector *vector3 = BoxedVector::fromFill(3, string3);

	{
		BoxedVector *emptyVector = BoxedVector::fromAppended({});

		ASSERT_EQUAL(emptyVector->length(), 0);
	}

	{
		BoxedVector *appendedVector = BoxedVector::fromAppended({vector1});
	...
	{
		BoxedVector *appendedVector = BoxedVector::fromAppended({vector1, vector2, vector3});
...
void testSetElement()
{
	BoxedString *testString = BoxedString::fromUtf8CString(u8"Test");
	BoxedVector *testVector  = BoxedVector::fromFill(5);

	ASSERT_EQUAL(testVector->elementAt(0), BoxedUnspecific::instance());
	ASSERT_EQUAL(testVector->setElementAt(0, testString), true);
	ASSERT_EQUAL(testVector->elementAt(0), testString);
	
	ASSERT_EQUAL(testVector->elementAt(4), BoxedUnspecific::instance());
...
void testCopy()
{
	auto testVector = BoxedVector::fromFill(5);

	for(unsigned int i = 0; i < 5; i++)
	{
		testVector->setElementAt(i, BoxedString::fromUtf8CString("TEST"));
	}

	{
		BoxedVector *wholeCopy = testVector->copy();
	...
	{
		BoxedVector *explicitWholeCopy = testVector->copy(0, 5);
	...
	{
		BoxedVector *firstTwoCopy = testVector->copy(0, 2);
	...
	{
		BoxedVector *lastTwoCopy = testVector->copy(3, 5);
	...
	{
		BoxedVector *emptyCopy = testVector->copy(3, 3);
...
void testReplace()
{
	auto *fromVector = BoxedVector::fromFill(5); 
	for(unsigned int i = 0; i < 5; i++)
	{
		fromVector->setElementAt(i, BoxedString::fromUtf8CString("TEST"));
	}
	
	BoxedDatum *destElements[5];
	for(unsigned int i = 0; i < 5; i++)
	{
		destElements[i] = BoxedString::fromUtf8CString("TEST");
	}

	{
		BoxedVector *toVector = BoxedVector::fromFill(5); 
		for(unsigned int i = 0; i < 5; i++)
		{
			toVector->setElementAt(i, BoxedString::fromUtf8CString("TEST"));
		}
...
void testFill()
{
	BoxedString *originalElement = BoxedString::fromUtf8CString("One");
	BoxedString *fillElement = BoxedString::fromUtf8CString("Two");

	{
		BoxedVector *testVector = BoxedVector::fromFill(5, originalElement);
