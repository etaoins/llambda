...
public:
	 ...
	 * @param  newChildNode  Non-empty tree to place at the specified index
	 */
	static InternalNode* fromSingleChild(std::uint32_t childIndex, DatumHashTree *newChildNode)
	{
...
DatumHashTree* DatumHashTree::assocAtLevel(DatumHashTree *tree, std::uint32_t level, AnyCell *key, AnyCell *value, DatumHash::ResultType hashValue, bool inPlace)
{
	...
	else if (tree->isLeafNode())
	{
		...
		// Create an internal node with a single value. Note we don't want to directly create a node with two values as
		// both leaf nodes might be assigned the same slot and require a second level internal node.
		auto childIndex = InternalNode::childIndex(level, leafNode->hashValue());
		InternalNode *newInternalNode = InternalNode::fromSingleChild(childIndex, leafNode->ref());
